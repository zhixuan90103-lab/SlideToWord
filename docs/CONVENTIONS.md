...
配套：[AGENTS.md](../AGENTS.md) · [SWIPE.md](./SWIPE.md) · [ENGINEERING.md](./ENGINEERING.md)
...
3. 否则：`atan2` 量化到 8 个 45°。已锁则偏角 &lt; `OCTANT_HOLD`（**约 28°**）不换向（须大于 22.5° 中线才有粘住）。  
4. 角度始终是起点中心 → 手指。禁止折线。  
5. 投影：`along = dot(δ, step) / (cell · |step|²)`。对角一步 = 1。  
6. **看见的条**用小数 `along` 跟手伸缩。**算进词的字母**用 `round(along)`。  
7. `cancel` 与划错相同：丢掉本次路径。
...
| 滑动 | 条沿锁定 8 向跟手变长（仍 0.75）；约 28° 内不换向；每新纳入一字母可轻震 + 预览更新 |
| 抬手成功 | 同色留下，直径改为 **0.65 格**；词表划掉；预览浮层消失 |
...
字母约 28px（约半格）。放大只 `transform` `.ws-glyph`，格子盒子不位移。`transform-origin: 50% 50%`。颜色立刻变白，不要用慢的颜色过渡冒充延迟。
