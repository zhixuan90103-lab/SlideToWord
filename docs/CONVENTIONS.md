...
词表底板高度 `hintsY`（默认 **0.60**），见 [TUNE.md](./TUNE.md)。预览胶囊不得挤开棋盘；锚点默认相对缝中心下移 24px。

**词表进出**：离场先上移再淡出（360ms，列表错开 28ms）；进场只淡入（300ms，错开 40ms）。找到的词走同一套离场。禁止只用 opacity 过渡、禁止进场再叠位移。
