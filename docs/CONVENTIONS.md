...
- 棋盘 DOM 是 `grid` 的只读投影。过波先写入终局格子；下落/落地缓冲只改同一套 `translateY + scale`，不换节点、不用第二套 WAAPI。
- 落地缓冲在同一条 rAF 里采样：轻下沉+轻压扁后回正，不上弹。闲置 `transform: none`，禁止 `translateZ` 交接。
- 下落/落地缩放以格子**下边缘**为原点（`transform-origin: 50% 100%`）。
